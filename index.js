...
import { getFirestore, doc, onSnapshot, setDoc, updateDoc, collection, addDoc } from 'firebase/firestore';

// Main App component for the landscaping project tracker
const App = () => {
    // --- State Variables ---
    ...
    // This effect runs once on component mount to handle user authentication
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            if (currentUser) {
                // If a user is already authenticated, set the user ID and state
                setUserId(currentUser.uid);
                setIsAuthenticated(true);
            } else {
                // If no user is found, reset state
    ...
    // This effect listens for changes in project data and payments in real-time
    useEffect(() => {
        // Only run if the user is authenticated and the userId is available
        ...
        // Function to set up initial project data if it doesn't exist
        ...
        // Set up real-time listener for the main project data document
        const unsubscribeProjectData = onSnapshot(projectDocRef, (docSnap) => {
            if (docSnap.exists()) {
        ...
        // Set up real-time listener for the payments collection
        ...
        // Cleanup listeners when the component unmounts or authentication state changes
        return () => {
    ...
    // Handles user login by checking the password and signing in anonymously
    ...
    // Updates the completion status of a project stage
    ...
    // Resets all project stages to incomplete
    ...
    // Adds a new note to the project data
    ...
    // Adds a new item to the list of excluded items
    ...
    // Adds a new payment and updates the total spend
    ...
    // Adds a new unexpected cost to the project data
    const handleAddUnexpectedCost = async (e) => {
        ...
        const projectDocRef = doc(db, 'artifacts', appId, 'users', userId, 'projectData', 'main');
        try {
            await updateDoc(projectDocRef, { unexpectedCosts: newUnexpectedCosts });
    ...
    // Updates the completion status of a provisional sum item
    ...
    // Updates the completion status of a prime cost item
    ...
    // Updates the completion status of an excluded item
    ...
    // Display a loading indicator while Firebase is authenticating
    if (loading) {
        return (
    ...
    // Display the main tracker dashboard
    return (
        <div className="min-h-screen bg-gray-100 p-4 font-sans text-gray-800">
            ...
            <main className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <section className="bg-white p-6 rounded-lg shadow-md col-span-1 md:col-span-2 lg:col-span-1">
